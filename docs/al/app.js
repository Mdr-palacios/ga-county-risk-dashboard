// AL County Risk Dashboard; independent analysis by Rosario Palacios.
// Read-only; no backend. Parallel to the GA and MS builds, adapted for AL factors.
...
// FIPS in AL data is the full 5-digit integer (01xxx); topo geometries also use integer ids 1xxx.
// Build a string-padded key for matching either form.
...
Promise.all([
  fetch('../data/al-counties-data.json').then(r => r.json()),
  fetch('../data/al-counties-topo.json').then(r => r.json()),
  fetch('../data/al-summary.json').then(r => r.json()),
]).then(([counties, topo, summary]) => {
  DATA.counties = counties;
  DATA.topo = topo;
  DATA.summary = summary;
  // Key by 5-digit string FIPS to match topojson path ids; also store integer keys for safety.
  DATA.byFips = {};
  counties.forEach(c => {
    const padded = String(c.fips).padStart(5, '0');
    DATA.byFips[padded] = c;
    DATA.byFips[c.fips] = c;
  });
  window.DATA = DATA;
  init();
}).catch(err => {
...
function renderKPIs() {
  ...
  const hispByTier = { Critical: 0, High: 0, Medium: 0, Low: 0 };
  for (const c of DATA.counties) {
    if (c.risk_tier in hispByTier) hispByTier[c.risk_tier] += (c.hispanic || 0);
  }

  // 287(g) by model (count agency rows, not counties)
  const typeRows = { TFM: 0, JEM: 0, WSO: 0 };
  for (const c of DATA.counties) {
    if (c['287g_types']) {
      typeRows.TFM += (c['287g_types'].TFM || 0);
      typeRows.JEM += (c['287g_types'].JEM || 0);
      typeRows.WSO += (c['287g_types'].WSO || 0);
    }
  }

  // Detention status counts
  const detGroups = { host: 0, adjacent: 0, none: 0 };
  for (const c of DATA.counties) {
    if (c.detention_status === 'Hosts ICE detention facility') detGroups.host++;
    else if (c.detention_status === 'Adjacent to detention facility') detGroups.adjacent++;
    else detGroups.none++;
  }

  // Hispanic concentration buckets
  const hispBuckets = { high: 0, mod: 0, low: 0 };
  for (const c of DATA.counties) {
    if (c.hisp_pct >= 10) hispBuckets.high++;
    else if (c.hisp_pct >= 5) hispBuckets.mod++;
    else hispBuckets.low++;
  }
  ...
  const cards = [
    ...
    {
      label: t('kpi.287g'), value: s.counties_with_287g, sub: t('kpi.287g.sub', { agencies: s.distinct_287g_agencies, rows: s.total_287g_rows }), accent: 'var(--risk-high)',
      context: t('kpi.287g.context'),
      viz: segBar([
        { label: t('kpi.287g.tfm'), v: typeRows.TFM, color: cssVar('--risk-critical') },
        { label: t('kpi.287g.jem'), v: typeRows.JEM, color: cssVar('--risk-high') },
        { label: t('kpi.287g.wso'), v: typeRows.WSO, color: cssVar('--risk-medium') },
      ]),
    },
    {
      label: t('kpi.detention'), value: detGroups.host + detGroups.adjacent, sub: t('kpi.detention.sub'), accent: 'var(--risk-critical)',
      context: t('kpi.detention.context'),
      viz: segBar([
        { label: t('kpi.detention.host'), v: detGroups.host, color: cssVar('--risk-critical') },
        { label: t('kpi.detention.adjacent'), v: detGroups.adjacent, color: cssVar('--risk-high') },
      ]),
    },
    {
      label: t('kpi.hisp'), value: fmt(s.hispanic_total), sub: t('kpi.hisp.sub', { pct: s.hispanic_pct_state, total: fmt(s.total_pop) }), accent: 'var(--color-primary)',
      context: t('kpi.hisp.context'),
      viz: microBars(tierKeys.map(tk => ({ label: t('tier.' + tk.toLowerCase()), v: hispByTier[tk], color: tierColors[tk] })), (n) => fmt(n)),
    },
    {
      label: t('kpi.hisp_concentration'), value: hispBuckets.high, sub: t('kpi.hisp_concentration.sub'), accent: 'var(--risk-high)',
      context: t('kpi.hisp_concentration.context'),
      viz: segBar([
        { label: t('kpi.hisp_concentration.high'), v: hispBuckets.high, color: cssVar('--risk-critical') },
        { label: t('kpi.hisp_concentration.mod'), v: hispBuckets.mod, color: cssVar('--risk-high') },
        { label: t('kpi.hisp_concentration.low'), v: hispBuckets.low, color: cssVar('--color-divider') },
      ]),
    },
  ];
...
function fillForFeature(d) {
  ...
  if (MAP_MODE === '287g_status') {
    if (row['287g_count'] >= 3) return cssVar('--risk-critical');
  ...
  if (MAP_MODE === 'detention') {
    ...
    if (row.detention_status === 'Adjacent to detention facility') return cssVar('--risk-high');
    return cssVar('--color-divider');
  }
  if (MAP_MODE === 'poultry') {
    if (row.score_poultry_belt >= 9) return cssVar('--risk-critical');
    if (row.score_poultry_belt >= 6) return cssVar('--risk-high');
    if (row.score_poultry_belt >= 3) return cssVar('--risk-medium');
    return cssVar('--color-divider');
  }
  if (MAP_MODE === 'hisp_pct') {
    const max = d3.max(DATA.counties, c => c.hisp_pct) || 1;
    const v = Math.min(1, row.hisp_pct / max);
...
function drawLegend() {
  ...
  if (MAP_MODE === 'risk_tier') {
    ...
    items = [
      { label: t('legend.multi_287g'), color: cssVar('--risk-critical') },
      { label: t('legend.one_287g'), color: cssVar('--risk-high') },
    ...
    items = [
      ...
      { label: t('legend.detention_adjacent'), color: cssVar('--risk-high') },
      { label: t('legend.detention_none'), color: cssVar('--color-divider') },
    ];
  } else if (MAP_MODE === 'poultry') {
    items = [
      { label: t('legend.poultry_heavy'), color: cssVar('--risk-critical') },
      { label: t('legend.poultry_mod'), color: cssVar('--risk-high') },
      { label: t('legend.poultry_some'), color: cssVar('--risk-medium') },
      { label: t('legend.poultry_none'), color: cssVar('--color-divider') },
    ];
  } else if (MAP_MODE === 'hisp_pct') {
    items = [
...
function showTip(e, d) {
  ...
  const detBit = row.detention_status === 'Hosts ICE detention facility' ? ' · ' + t('tip.detention_host')
                 : row.detention_status === 'Adjacent to detention facility' ? ' · ' + t('tip.detention_adjacent')
                 : '';
  tip.innerHTML = `<div class="t-name">${row.county} ${t('detail.county_suffix')}</div><div class="t-meta">${tierLabel} · ${t('tip.score')} ${row.risk_total} · ${ag}${detBit}</div>`;
...
function renderDetail(c) {
  ...
  else if (c.detention_status === 'Adjacent to detention facility') tags.push({ t: t('detail.tag.detention_adjacent'), cls: 'warn' });
  if (c.score_poultry_belt >= 9) tags.push({ t: t('detail.tag.poultry_heavy'), cls: 'warn' });
  else if (c.score_poultry_belt >= 5) tags.push({ t: t('detail.tag.poultry_mod'), cls: '' });
  if (c.hisp_pct >= 10) tags.push({ t: t('detail.tag.hisp_high', { pct: c.hisp_pct.toFixed(1) }), cls: 'warn' });
  if (tags.length === 0) tags.push({ t: t('detail.tag.none'), cls: 'ok' });

  const maxBars = [
    { label: t('detail.score.287g'), val: c.score_287g, max: 30 },
    { label: t('detail.score.detention'), val: c.score_detention, max: 25 },
    { label: t('detail.score.hisp'), val: c.score_hispanic, max: 20 },
    { label: t('detail.score.pop'), val: c.score_population, max: 10 },
    { label: t('detail.score.poultry'), val: c.score_poultry_belt, max: 10 },
    { label: t('detail.score.hb56'), val: c.score_base, max: 5 },
  ];

  const tierLabel = t('tier.' + c.risk_tier.toLowerCase());
  const regionName = t('region.' + c.region) || c.region;

  // Build agency list display
  const agenciesHtml = (c['287g_agencies'] && c['287g_agencies'].length)
    ? c['287g_agencies'].map(a => `${a.agency} (${(a.types || []).join('/')})`).join('; ')
    : '';
  ...
  el.innerHTML = `
    <div class="panel-head">
      <div>
        ...
        <p class="sub">${t('detail.fips')} ${c.fips} · ${t('detail.pop')} ${fmt(c.total_pop)} · ${regionName}</p>
      </div>
    ...
    <div class="stat-grid">
      ...
      <div class="row"><span class="l">${t('detail.stat.hb56')}</span><span class="v" style="font-size: var(--text-sm); font-family: var(--font-body); font-weight: 600;">${c.hb56_sb53_status}</span></div>
      ...
      <div class="row"><span class="l">${t('detail.stat.poultry')}</span><span class="v" style="font-size: var(--text-sm); font-family: var(--font-body); font-weight: 600;">${c.poultry_status}</span></div>
    </div>

    ${agenciesHtml ? `<p style="font-size: var(--text-xs); color: var(--color-text-muted); margin-bottom: var(--space-3);"><strong style="color: var(--color-text);">${t('detail.287g_agencies')}</strong> ${agenciesHtml}</p>` : ''}
...
function renderTierBars() {
  ...
  `).join('') + `
    <div style="margin-top: var(--space-5); display: grid; grid-template-columns: 1fr 1fr; gap: var(--space-4); padding-top: var(--space-4); border-top: 1px solid var(--color-divider);">
      <div>
        <div style="font-size: var(--text-xs); font-weight: 600; color: var(--color-text-muted); letter-spacing: 0.04em; text-transform: uppercase; margin-bottom: var(--space-2);">${t('tier.pop_share') || 'Population share'}</div>
        ${tiers.map(tt => `<div style="display:flex;align-items:center;gap:var(--space-2);font-size:var(--text-xs);margin-bottom:4px;"><div style="width:8px;height:8px;border-radius:2px;background:${tt.color}"></div><span style="width:60px">${t('tier.' + tt.tier.toLowerCase())}</span><span style="font-family:var(--font-mono);font-weight:600;">${(tt.pop / totalPop * 100).toFixed(0)}%</span></div>`).join('')}
      </div>
      <div>
...
function render287gBars() {
  // Buckets: counties with multi-agency, single TFM, single JEM/WSO, none
  const buckets = [
    { key: 'multi', label: t('p287g.bar.multi') || 'Multiple agencies', count: 0, color: cssVar('--risk-critical') },
    { key: 'tfm', label: t('p287g.bar.tfm') || 'TFM only', count: 0, color: cssVar('--risk-high') },
    { key: 'jail', label: t('p287g.bar.jail') || 'Jail only (JEM/WSO)', count: 0, color: cssVar('--risk-medium') },
    { key: 'none', label: t('p287g.bar.none') || 'No 287(g)', count: 0, color: cssVar('--color-divider') },
  ];
  for (const c of DATA.counties) {
    if (c['287g_count'] === 0) { buckets[3].count++; continue; }
    if (c['287g_count'] >= 2) { buckets[0].count++; continue; }
    const types = c['287g_types'] || {};
    if (types.TFM) buckets[1].count++;
    else buckets[2].count++;
  }
  const max = d3.max(buckets, g => g.count) || 1;
  const el = document.getElementById('287g-bars');
  el.innerHTML = buckets.map(g => `
    <div class="bar-row">
      <span class="lab" style="width: 168px; font-size: var(--text-xs);">${g.label}</span>
      <div class="track"><div class="fill" style="width: ${(g.count / max) * 100}%; background: ${g.color}; color: ${g.key === 'none' ? 'var(--color-text)' : 'white'};">${g.count}</div></div>
    </div>
  `).join('');
}
...
function renderRegionBars() {
  ...
  const groups = REGION_ORDER.map(rid => {
    const cs = DATA.counties.filter(c => c.region === rid);
    if (!cs.length) return null;
    const avg = cs.reduce((s, c) => s + c.risk_total, 0) / cs.length;
    const hisp = cs.reduce((s, c) => s + c.hispanic, 0);
    const critHigh = cs.filter(c => c.risk_tier === 'Critical' || c.risk_tier === 'High').length;
    return { rid, label: t('region.' + rid) || rid, avg, hisp, critHigh, count: cs.length };
  }).filter(Boolean);

  const max = d3.max(groups, g => g.avg) || 1;
  const el = document.getElementById('region-bars');
  el.innerHTML = groups.map(g => {
    const color = g.avg >= 50 ? cssVar('--risk-critical')
                : g.avg >= 35 ? cssVar('--risk-high')
                : g.avg >= 20 ? cssVar('--risk-medium')
                : cssVar('--risk-low');
    return `
      <div class="bar-row" style="margin-bottom: 10px;">
        <span class="lab" style="width: 200px; font-size: var(--text-xs);">${g.label}</span>
        <div class="track"><div class="fill" style="width: ${(g.avg / max) * 100}%; background: ${color}; color: white;">${g.avg.toFixed(1)}</div></div>
      </div>
      <div style="display:flex;gap:var(--space-3);font-size:var(--text-xs);color:var(--color-text-muted);margin: -4px 0 var(--space-2) 208px;">
        <span>${g.count} ${t('region.counties_suffix') || 'counties'}</span>
        <span>${g.critHigh} ${t('region.crit_high_suffix') || 'Critical/High'}</span>
        <span>${fmt(g.hisp)} ${t('region.hispanic_suffix') || 'Hispanic'}</span>
      </div>
    `;
  }).join('');
}
...
function renderTable() {
  ...
  tbody.innerHTML = rows.map(c => {
    ...
    const isAdj = c.detention_status === 'Adjacent to detention facility';
    return `
      <tr data-fips="${c.fips}">
        ...
        <td>${c['287g_count'] > 0 ? `<span class="tag warn" style="padding:2px 6px;font-size:11px;">${c['287g_status']}</span>` : '<span class="dash">·</span>'}</td>
        <td>${isHost ? `<span class="tag danger" style="padding:1px 5px;font-size:10px;">${t('tbl.cell.host') || 'host'}</span>` : isAdj ? `<span class="tag warn" style="padding:1px 5px;font-size:10px;">${t('tbl.cell.adj') || 'adj.'}</span>` : '<span class="dash">·</span>'}</td>
        <td style="font-size: var(--text-xs); color: var(--color-text-muted);">${t('region.' + c.region) || c.region}</td>
        <td class="num">${fmt(c.total_pop)}</td>
  ...
  if (pill) pill.textContent = `${rows.length} / ${DATA.counties.length}`.trim();
}
...
function filterRows() {
  ...
  const det = document.getElementById('filter-detention').value;
  ...
  return DATA.counties.filter(c => {
    ...
    if (det === 'host' && c.detention_status !== 'Hosts ICE detention facility') return false;
    if (det === 'adjacent' && c.detention_status !== 'Adjacent to detention facility') return false;
    if (det === 'none' && c.detention_status !== 'No nearby detention') return false;
...
function sortRows(rows) {
  return [...rows].sort((a, b) => {
    ...
    if (SORT_KEY === 'detention_status') {
      const score = (s) => s === 'Hosts ICE detention facility' ? 2 : s === 'Adjacent to detention facility' ? 1 : 0;
...
function bindFilters() {
  ...
  document.getElementById('filter-detention').addEventListener('change', renderTable);
...
(function () {
  const SIGNUP_KEY = 'al-dash-signup-dismissed';
  const COUNT_KEY = 'al-dash-county-clicks';
  const BANNER_KEY = 'al-dash-banner-dismissed';
